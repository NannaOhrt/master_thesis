...
explanation of the parameters. Then the study is performed by changing the parameters one by one, to illustrate the 
...
\end{figure}
\noindent
Then we will continue the study of the parameters effects by looking into shifts in the $\beta$ value in the SABR model.
...
of $\nu$. Which makes sense, when we think of the interpretation of the parameter described earlier.
...
\end{figure}
\noindent
Finally we will investigate how the forward price, F, affects the volatility smile in the SABR model. 
...
\\\\
We will apply the approach, where we chose some fixed $\beta$'s and then estimate $\alpha$ using \autoref{sigma_atm}  and then
setup a minimization problem that minimizes $\rho$ and $\nu$. We chose this approach since earlier in Chapter \ref{invest_sabr}
we noticed that changes in $\beta$ and $\rho$ gave some of the same effect on the volatility smile. Therefor we chose to fix  $\beta$, for different
...
where the ATM SABR-formula is estimated numerically. The ATM SABR-formula is listed in \autoref{sigma_atm} \cite{Smile}.
...
namely [1Y,2Y,3Y,5Y,7Y,10Y,12Y,15Y,20Y,30Y]. Below in \autoref{tab:farward_parm} the described forward rates are 
presented. The development of the forward rates over the different tenors are illustrated
in \autoref{fig:forward_plot} below. From the plot we see that the level of the forward rates are higher for
...
\\\\
To end this section we will look at estimated parameters depicted by the volatility smile. 
We choose to look a the volatility smile, for estimated values where $\beta$ was fixed to be 0.5, 
since this is the most common choice for $\beta$. We depicted the combination where the tenor is 10 years, in large format in 
\autoref{fig:10y_plot_smile} below and all combinations of the swaption volatility smile with expiry in 10 years, and the 
various tenors are depicted in \autoref{fig:10Y1Y_} to \autoref{fig:10Y10Y_} below. 
...
\autoref{tab:beta_0.5} and \autoref{tab:beta_1}, are relative high. This align with the steepness or level we see the in the 
...
But we also see for the tenor-expiry combinations for longer tenors does not have the same steepness in the level in 
the volatility smile. For the longer durations tenor combinations we see a more symmetric volatility smile around the
ATM strike. We also note the the changes in the volatility smile i larger for the OTM strike, than for the ITM strikes. 
\\\\
Then we should ask ourself how does the  steepness or level we see the in the 
volatility smile in the market data affect the price of a swaption. When we determine swaption prices, we should also consider 
which sensitivities that are related to the price of the swaption. The above analysis points at there could 
be a price-sensitivity related to $\nu$. Therefor the next step in the analysis is to look closer at the models sensitivities. 
