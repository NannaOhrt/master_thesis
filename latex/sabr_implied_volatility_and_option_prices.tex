...
\\\\
We will proceed with the approach, where we chose some fixed $\beta$'s and the estimate $\alpha$ using \autoref{sigma_atm}  and the
...
Then remind yourself of the dynamic of the forward rate in the SABR model, listed in \autoref{f_dyn}. 
...
To estimate $\alpha$ we will use the method proposed in  (Hagan et al., 2002), 
where the ATM SABR-formula is inverted numerically, where the ATM SABR-formula is listed in \autoref{sigma_atm} \cite{Smile}.
...
\end{align}
Before we are ready to estimate the parameters numerically, lets first take a short look at the required 
data to performed the estimation study. As mentioned we need the current forward rates for any combination 
of expiry and tenor. In this analysis we will consider data where the expiry is 10 years and has various tenors 
...
\end{figure}
\noindent
So we have chosen the method where we chose a fixed $\beta$, so we chose the fixed $\beta$'a to be 0.5 and 1.
...
volatility smile in the market data. The volatility smile tends to be more convex we $\nu$ is large and the is clearly 
the case here. This tendency appearers also i most of the volatility smile despited in \autoref{fig:10Y1Y_} to \autoref{fig:10Y10Y_}.
\\\\
...
be a sensitive related to $\nu$. Therefor the next step is to look closer at the models sensitivities. 
\begin{figure}[H]
    \centering
