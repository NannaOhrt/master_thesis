...
\begin{align*}
    \Pi(t)_{\text{Payer swaption}} = \sum_{i=1}^{mn} \frac{L}{m} B(0,T_i)\Big[ S_F N(d_1) - S_K N(d_2)\Big] \quad \text{for} \quad T_i= T+i/m  
\end{align*}
where
\begin{align*}
...
zero, $\sigma$ is the volatility of the forward swap rate. The term $\sum_{i=1}^{mn} \frac{L}{m} B(0,T_i)$ 
...
\begin{align*}
    L A \Big[S_K N(-d_2) - S_F N(-d_1)\Big]
\end{align*}
\\\\
