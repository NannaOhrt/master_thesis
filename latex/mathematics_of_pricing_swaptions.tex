...
which is know as the swap rate. This fixed rate is exchange for some floating rate, such as the LIBOR rate or the EURIBOR rate. 
...
\begin{align}
    ...
    \label{irs}
\end{align}
where we have that $L(T_{i-1},T_i)$ is the spot rate, which could be the LIBOR spot rate or the EURIBOR spot rate.  It is also assumed
...
\begin{align}
    ...
    \label{valueirs}
\end{align}
Moving forward we simplify \autoref{valueirs} in the below Proposition \ref{simple}
\begin{proposition}
