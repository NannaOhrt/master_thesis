...
is purpose is the price European swaption. Likewise for swaps, there is also different type of swaption. 
...
and receive a floating rate on the swap that will expire in n year starting on T years.
...
be assumed that each fixed payment on the swap is the fixed rate time L/m. Next we suppose that 
...
of a year, is paid a throughout the life of the swap. The payment of the swap, have the following 
payments dates 
\begin{align*}
    T_1,T_2,...,T_{mn}
\end{align*}
Let be reminded that a swaption is a option on the swap rate, which is the one there generated the payoffs.
Then we formulated the price of the payer swaption, this is illustrated below
\begin{align*}
    \sum_{i=1}^{mn} \frac{L}{m} B(0,T_i)\Big[ S_F N(d_1) - S_K N(d_2)\Big] \quad \text{for} \quad T_i= T+i/m  
\end{align*}
where
\begin{align}
    d_1 & = \frac{1}{\sigma \sqrt{T}} \Big[ \ln(\frac{S_F}{S_k})+\sigma^2(\frac{T}{2})\Big] \\
    d_2 & = d_1-\sigma \sqrt{T}
\end{align}
Where N is the cumulative distribution for the $N [0,1]$ distribution, $S_F$ is the forward swap rate time
zero, $\sigma$ is the volatility og the forward swap rate. The term $\sum_{i=1}^{mn} \frac{L}{m} B(0,T_i)$ 
...
\begin{align}
    A= \frac{1}{m} \sum_{i=1}^{mn} B(0,T_i)
    \label{a_simple}
\end{align}
Hence we have that the value of the swaption can be expressed as in \autoref{value_swaption}
\begin{align}
    L N \Big[S_F N(d_1) - S_K N(d_2) \Big]
    \label{value_swaption}
\end{align}
...
\end{align}
Finally we can end this section 