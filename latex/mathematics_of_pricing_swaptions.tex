...
time period $[t_{i-1},T_i]$. So for the i'th coupon the return is equal tol $r_i$, and the face value 
is K, hence we have that 
\begin{align*}
    c_i = r_i(T_i-T_{i-1})K
\end{align*}
Where for stand coupon, the time intervals will be equally spaced, which means that 
\begin{align*}
    T_i = T_0 + i \delta
\end{align*}
...
of xIBOR rates, one that we we encounter later is EURIBOR rate. Which is a rate the 
European Banking Federation sets every business day. 
\\\\
These different type of xIBOR rates are sets differently, but the use the money market convention. 
...
loss is generality assume that K=1 and insert the definition \ref{def:spot} of the LIBOR rate 
to obtain
\begin{align*}
    c_i &= (T_i-T_{i-1})L(T_{i-1},T_i)K \\
        &= \delta L(T_{i-1},T_i) \\
        &= \delta \frac{1- p(T_{i-1},T_i)}{\delta p(T_{i-1},T_i)} = \frac{1}{p(T_{i-1},T_i)}-1
\end{align*}
...
\begin{assumption}\label{def:market}
    ...
    \begin{itemize}
        ...
        \item For each fixed, the bond price p(t,T) is differentiable w.r.t time of maturity T \cite{Bjork}
    \end{itemize}
\end{assumption}
