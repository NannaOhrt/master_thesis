...
\end{definition} 
\noindent
Before moving forward we will look at the cashflow for a zero coupon bond. The illustration in \autoref{zcb_fig} show that a time t,
the principal payment is made a the price $P(t,T)$ and at maturity T the principal repaid. So the cash illustrated 
what the price $P(t,T)$ of one dollar repaid at time T.  
\begin{figure} 
\centering
\begin{tikzpicture}[font=\small]
    ...
    \draw (11.5,-0.2) -- (11.5,0.5) node[below] at (11.5,-0.3) {T (Maturity)};
...
\end{tikzpicture}
\caption{Cash Flow Structure of a Zero Coupon Bond}
\label{zcb_fig}
\end{figure}
