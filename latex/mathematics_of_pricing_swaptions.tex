...
\end{definition} 
\noindent
Before moving forward we will look at the cashflow for a zero coupon bond. The illustration show that a time t,
the principal payment is made a the price $P(t,T)$ and at maturity T the principal repaid.
\begin{center}
...
\begin{tikzpicture}
    ...
    \addplot[domain=0:10, samples=1, thick] {x^(0.41)};
    \end{axis}
\end{tikzpicture}
\end{center}
...
\begin{center}
    \begin{tikzpicture}
        \begin{axis}[
            ...
            xlabel style={at={(axis description cs:1,-0.02)},anchor=north},
            ylabel style={at={(axis description cs:-0.13,0.3)},anchor=west},
        ]
        \addplot[smooth, tension=0.5] plot coordinates {
            (0,0)
            (1,1)
            (2,1.7)
            (3,2.8)
