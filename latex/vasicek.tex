...
formulas for bond pricing. First some word on how this in done and the a more the approach. 
...
\end{align}
 Combining this we get the formula for pricing bonds using the Vasicek model, which present the 
\begin{proposition}
...
Likewise for section 3, some choices is made when bond prices is calculated using the Vasicek model. One important 
chose is that the volatility, $\sigma$ is determined to be a positive constant. But one question you should asked 
yourself is, are volatility constant over a given time period?. Later another model will be introduced, 
namely the SABR model. The SABR model is a stochastic volatility model, which is used to estimate implied volatility curve, 
but the model do not formula a direct option pricing formula. But the estimate implied volatility curve, can be used in 
the Black Scholes model we covered Earlier to price swaption. Before introducing the SABR model we will look at 
the constant volatility there is inherit in the Black Scholes model. 
